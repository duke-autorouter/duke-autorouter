...
export async function check(caseId: string, dir: string) {
  ...
  const fixtureHashes = Object.fromEntries(
    await Promise.all(
      Object.entries(item.files).map(async ([name, original]) => [
        name,
        {
          expected: sha(original),
          actual: sha(
            await readFile(join(dir, name), "utf8").catch(() => "MISSING"),
          ),
        },
      ]),
    ),
  ) as Record<string, { expected: string; actual: string }>;
  const fixturesUnchanged = Object.values(fixtureHashes).every(
    (h) => h.expected === h.actual,
  );
  if (item.kind === "coding")
    return {
      fixtureHashes,
      fixturesUnchanged,
      acceptance: "node --test verify.mjs",
      rubric: item.rubric,
    };
  ...
  return {
    ...
    rubric: item.rubric,
    humanReview: { accepted: null, notes: "" },
  };
}
