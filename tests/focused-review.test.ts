...
const claim = (v: string) => {
  const selected =
    v === "pass" ? "supported" : v === "fail" ? "contradicted" : v;
  return {
    type: "choice",
    choice: selected,
    confidence: 1,
      ...
      return Response.json({
        answers: Object.fromEntries(
          Object.keys(body.questions).map((id) => [
            id,
            verdicts[id] ??
              (id.startsWith("claim_") ? claim("pass") : answer("pass")),
          ]),
        ),
...
test("focused evidence preserves literal references and never uses earlier output as source", () => {
  ...
  const source =
    "Unrelated background. ".repeat(150) + "Workshop owner: not assigned.";
  const windows = sourceWindows(
    "Workshop owner Ana",
    [{ path: "brief.md", text: source }],
    true,
  );
  assert.ok(windows.some((w) => w.text.includes("not assigned")));
  assert.ok(
    windows.every(
      (w) => source.slice(w.offset, w.offset + w.text.length) === w.text,
    ),
  );
});
test("a specific failed claim survives broad passing judgments and does not solicit another opinion", async (t) => {
  ...
  assert.match(
    checks.find((c) => c.name === "Jev: claim_0")!.detail,
    /checklist.md.*Ana/,
  );
...
test("uncertainty receives exactly one expanded pass, with both calls billed as review", async (t) => {
  ...
  assert.equal(
    f.store
      .events("t")
      .filter((e) => e.kind === "usage_started" && e.data.role === "review")
      .length,
    2,
  );
});
test("continued uncertainty and passage overflow cannot become verified success", async (t) => {
  ...
  assert.equal(
    checks.find((c) => c.name === "Focused review coverage")?.status,
    "unverified",
  );
});
test("an uncorroborated broad failure stays neutral when a claim is malformed", async (t) => {
  ...
  assert.equal(
    checks.find((c) => c.name === "Jev: brief")?.status,
    "unverified",
  );
  assert.equal(
    checks.find((c) => c.name === "Jev: claim_0")?.status,
...
test("literal requirements are checked separately and findings cannot waive them", async (t) => {
  ...
  assert.equal(
    checks.find((c) => c.name === "Jev: requirement_1")?.status,
    "failed",
  );
  assert.match(
    f.calls[0].questions.requirement_1.instructions,
    /Label proposals/,
  );
});
...
test("ownership extraction separates a person-to-item assertion from role and status", () => {
  assert.deepEqual(
    ownershipClaim(
      "- [ ] Internal workshop before invitations — Ana (delivery); unresolved",
    ),
    { item: "Internal workshop before invitations", person: "Ana" },
  );
  assert.equal(
    ownershipClaim("- [ ] Readiness review — TBD; unresolved"),
    undefined,
  );
  assert.equal(
    ownershipClaim("Proposal: review meeting owner: Sam."),
    undefined,
  );
});
