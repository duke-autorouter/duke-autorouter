...
All 255 local macOS tests passed, with zero failures or skips, and TypeScript passed. Three fresh Jev-only dependent-instruction probes accepted the compliant result and rejected both an invented promise and a forbidden greeting. They cost $0.000400 with zero unreconciled requests. The cases were frozen at `5531809` before calls; the later smart-apostrophe correction has local regression coverage but no separate live probe. See the [raw receipt](evidence/dependent-review-0111-20260923.json). These checks do not establish general semantic accuracy or calibrated difficulty selection. The independent Linux report lists 236 passes and nine platform-bound failures out of 248 tests; the raw summary is needed to confirm the remaining three cases. We do not reinterpret those counts as a successful macOS run.
